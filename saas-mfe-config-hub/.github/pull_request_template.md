...
5. **Passing Pull-Request-Builder (PRB)**
    ...
    Receive a passing green run from the [pull request builder](https://construct.identitysoon.com/view/UI/job/UI-Admiral-PRB/). The PRB verifies that all the tests, linting, and build checks pass.

6. **Update Jira Issue**

    If the Jira issue has only one pull request against this repository then set the `Releasable Component: UI-ADMIRAL` field. In addition set the `Deploy Risk: Low|Medium|High` field depending on how risky this change is to cause a regression when deployed.

    If the Jira issue has multiple pull requests against other repositories then create a sub-task for each repository affected. On the sub-task for this repository set the `Releasable Component: saas-mfe-config-hub` field, and the `Deploy Risk: Low|Medium|High` field depending on how risky this change is to cause a regression when deployed. Use the parent's Jira key for the PR title and all other communication about the issue. The sub-task will track when this change is deployed.
