...
const lifecycles = singleSpaAngular({
	bootstrapFunction: async singleSpaProps => {
		return platformBrowserDynamic([
			...
			{
				provide: AUTH_CONTEXT_DI_TOKEN,
				useValue: await singleSpaProps[APP_SHELL_SERVICE_NAME].getAuthContextV1()
			},
			{
			...
			{
				provide: URL_CONTEXT_DI_TOKEN,
				useValue: await singleSpaProps[APP_SHELL_SERVICE_NAME].getAppShellUrlsProvider()
			},
			{ provide: MFE_INFO_DI_TOKEN, useValue: singleSpaProps[MFE_INFO_NAME] }
		]).bootstrapModule(AppModule);
	},
	template: '<app-config-hub-root />',
	Router,
	NavigationStart,
	NgZone
});
