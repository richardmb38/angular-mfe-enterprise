...
6. **Update Jira Issue**
    ...
    If the Jira issue has multiple pull requests against other repositories then create a sub-task for each repository affected. On the sub-task for this repository set the `Releasable Component: UI-ADMIRAL` field, and the `Deploy Risk: Low|Medium|High` field depending on how risky this change is to cause a regression when deployed. Use the parent's Jira key for the PR title and all other communication about the issue. The sub-task will track when this change is deployed.
